(ns cloudinary.main
  "Kotodama WASM entrypoint for the cloudinary clean-room actor (L4) — Clojure port.
  ...
  facts (`cloudinary.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "cloudinary")
(def tier "L4")
...
(def entity-specs
  [{:entity "Resource"       :plural "resources"     :id-prefix "cloudina_res"
    :fields [:assetId :publicId :resourceType :type :format :bytes :width :height :version :url :secureUrl]
    :required [:resourceType :type]
    :coerce {:bytes :int :width :int :height :int :version :int}
    :refs {}}
   {:entity "Folder"         :plural "folders"       :id-prefix "cloudina_fol"
    :fields [:name :path :externalId]
    :required [:name :path]
    :coerce {}
    :refs {}}
   {:entity "Transformation" :plural "transformations" :id-prefix "cloudina_tra"
    :fields [:name :named :used :allowedForStrict]
    :required [:name :named]
    :coerce {:named :bool :used :bool :allowedForStrict :bool}
    :refs {}}
   {:entity "UploadPreset"    :plural "uploadpresets" :id-prefix "cloudina_upl"
    :fields [:name :unsigned :externalId :live]
    :required [:name :unsigned]
    :coerce {:unsigned :bool :live :bool}
    :refs {}}
   {:entity "Tag"            :plural "tags"          :id-prefix "cloudina_tag"
    :fields [:name]
    :required [:name]
    :coerce {}
    :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"cloudinary.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "cloudinary-compat" :tier tier :entities entities} 200])
...
(defn start! [] :cloudinary-compat/ready)
