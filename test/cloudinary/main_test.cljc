(ns cloudinary.main-test
  "Contract + behavioral test for the cloudinary-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "cloudinary-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))
